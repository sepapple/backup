...
//    double lon = 0;
    ...
    //加速度に関連する変数
    int counter = 0;
    int chunks = 30; //まとめて送るデータを指定
    long last_time = 0;
    long current_time = 0;
    private static final int ALLAY_NUM = 10; //配列に入れる数
    private static final int MEASURE_MILL = 100; //0.1秒間隔で計測
    List<JSONObject> SendData = new ArrayList<JSONObject>(); //送信するリスト構造(一回分のデータはJSONでまとめる)
    ...
    //Evaluate_Swarm
    //String contractAddress = "0xca6Ac740b458154C64c7F03B1cb4A59876960561";
    //webinterface
    String contractAddress = "0xeA70b752595370A68474c7DED05Bd135F557f494";
    ...
    Credentials credentials = Credentials.create("48a6f74aca2fae8c50dd2ef29ea5804da7cf148235dd071456d29db313beba7a");  //資格情報
...
//        lon = temp_lon;
    ...
    public void SensorValue() {
                            ...
                            try {
                                ...
                                SendData.add(json);
                                    if(counter >= chunks) {
                                        DataRegist(SendData);
                                        counter = 0;
                                        SendData = new ArrayList<JSONObject>();
                                    }
                            }catch (Exception e){
                                mainHandler.post(() -> {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        TransactionReceipt transactionReceipt = myContract.UserStart(new BigInteger(ANDROID_ID)
                                , new BigInteger(ID[0]), BigInteger.valueOf(send_time), new BigInteger(ID[1]),lat,lon).send();
        ...
        //Regist関数の非同期処理
        public void DataRegist (List<JSONObject> sendData) {
            new Thread(new Runnable() {
                public void run() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        TransactionReceipt transactionReceipt = myContract.UserFinish(new BigInteger(ANDROID_ID)
                                , BigInteger.valueOf(send_time),lat,lon).send();
