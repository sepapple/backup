...
#ファイルからデータの抽出
f =  open('/home/pi/code/Trial_Swarm/CarContract.json','r')
...
def sendData():
    global URL,peripheral_ID,w3,myContract,lat,lon,accel_x,accel_y,accel_z,key
    # temp_accel_x = dict(zip(key,accel_x))
    # temp_accel_y = dict(zip(key,accel_y))
    # temp_accel_z = dict(zip(key,accel_z))
    
    send_text = {}
    send_text["time"] = round(time.time()*1000)
    send_text["lat"] = lat
    send_text["lon"] = lon
    send_text["accel_x"] = accel_x
    send_text["accel_y"] = accel_y
    send_text["accel_z"] = accel_z
    ...
    #gethへ送信
    regist = myContract.functions.CarRegistData(peripheral_ID,sendtime,high,low).transact({'from':w3.eth.accounts[1]})
    print("regist関数起動")
