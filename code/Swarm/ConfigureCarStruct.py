...
def main_program(args):
    ...
    result = myContract.functions.CarGetData(peripheral_ID,int(args[1])).call()
    # myContract.functions.CarGetData(peripheral_ID,int(args[1])).call({'from': w3.eth.accounts[0]})
    # result = myContract.functions.CarGetData(ANDROID_ID,int(args[1])).call()
